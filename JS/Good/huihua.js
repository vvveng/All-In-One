...
const targetAppId = 'iRingo.TestFlight';  // 仅切换这个应用的会话
    ...
    Object.keys(apps).forEach((appId) => {
      ...
      nextSession.datas.forEach((_data) => {
        ...
        if (switchkeys[key]) {
          ...
          switchkeys[key] = { val: val, session: nextSession };
        }
        $.setdata(val, key);
      });
  ...
  return new Promise((resolve) => {
    ...
    if ($.isSurge()) {
      ...
      if (!$.CFG_isSilent || $.CFG_isSilent === 'false') {
        $notification.post($.name, $.subt, msgContent, options);
      } else {
        $.log('', ...$.desc);
      }
    } else {
      if (!$.CFG_isSilent || $.CFG_isSilent === 'false') {
        $.msg($.name, $.subt, msgContent);
      } else {
        $.log('', ...$.desc);
      }
    }
